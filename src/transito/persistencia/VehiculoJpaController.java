...
public class VehiculoJpaController implements Serializable {
    ...
    public void create(Vehiculo vehiculo) throws PreexistingEntityException, Exception {
        ...
        try {
            ...
            Propietario cedula = vehiculo.getCedula();
            if (cedula != null) {
                cedula = em.getReference(cedula.getClass(), cedula.getCedula());
                vehiculo.setCedula(cedula);
            }
            em.persist(vehiculo);
            if (cedula != null) {
                cedula.getVehiculoList().add(vehiculo);
    ...
    public void edit(Vehiculo vehiculo) throws NonexistentEntityException, Exception {
        ...
        try {
            ...
            Propietario cedulaOld = persistentVehiculo.getCedula();
            Propietario cedulaNew = vehiculo.getCedula();
            if (cedulaNew != null) {
                cedulaNew = em.getReference(cedulaNew.getClass(), cedulaNew.getCedula());
                vehiculo.setCedula(cedulaNew);
            }
            vehiculo = em.merge(vehiculo);
            if (cedulaOld != null && !cedulaOld.equals(cedulaNew)) {
                cedulaOld.getVehiculoList().remove(vehiculo);
                cedulaOld = em.merge(cedulaOld);
            }
            if (cedulaNew != null && !cedulaNew.equals(cedulaOld)) {
                cedulaNew.getVehiculoList().add(vehiculo);
    ...
    public void destroy(String id) throws NonexistentEntityException {
        ...
        try {
            ...
            Propietario cedula = vehiculo.getCedula();
            if (cedula != null) {
                cedula.getVehiculoList().remove(vehiculo);
                cedula = em.merge(cedula);
            }
            em.remove(vehiculo);
            em.getTransaction().commit();
